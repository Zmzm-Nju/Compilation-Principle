		...
		if(strcmp(child_2->firstChild->nextSibling->nextSibling->token_type, "VarList")==0)
		{
			...
			*/
			while(Var_List->firstChild->nextSibling != NULL)
			{
				...
				table_item = VarDec(Param_Dec->firstChild->nextSibling, specifier_type);
				if(find_items_in_global_table(table_item->item_name))
					printf("Error type 3 at Line %d: Redefined variable \"%s\".\n", Param_Dec->firstChild->nextSibling->col, item->item_name);
				else 
				{
					FieldList temp = (FieldList)malloc(sizeof(struct FieldList_));
					strcpy(temp->name, table_item->item_name);
					temp->type = table_item->item_type;
					temp->next = NULL;
					if(tail == NULL)
						head = tail = temp;	
					else
					...
					table_item->next_item = Symbol_Table;
					Symbol_Table = table_item;
				} 
...
Type Exp(struct Token* node)
{
	...
	*/
	...
	{
		Type function_type = Exp(node->firstChild);
		//Exp ASSIGNOP Exp
		if(strcmp(node->firstChild->nextSibling->token_type, "ASSIGNOP")==0)
		{
			...
			if(!check_type(function_type, type_3))
			{
				printf("Error type 5 at Line %d: Type mismatched for assignment.\n", node->col);
				return NULL;
			}
		...
		*/
		...
		{
			Type type_3 = Exp(node->firstChild->nextSibling->nextSibling);
			if((!check_type(int_type, function_type))||(!check_type(int_type, type_3))||(!check_type(function_type, type_3)))
			{
		...
		*/
		...
		{
			Type type_3 = Exp(node->firstChild->nextSibling->nextSibling);
			if(check_type(function_type, type_3) == 0 ||
				(!check_type(int_type, function_type))&&(!check_type(float_type, function_type))||
				(!check_type(int_type, type_3))&&(!check_type(float_type, type_3)))
			{
				printf("Error type 7 at Line %d: Type mismatched for operands.\n", node->col);
				return NULL;
			}
			else return function_type;
		}
		//Exp LB Exp RB	
		...
		{
			Type structure_type = function_type;
...
//其中read函数没有任何参数,返回值为int型(即读入的整数值),
...
void init_table()
{
	...
	Type function_type = (Type)malloc(sizeof(struct Type_));
	function_type->kind = FUNCTION;
	function_type->u.function.funcType = return_type;
	function_type->u.function.paramNum = 0;
	function_type->u.function.params = NULL;
	Item table_item = (Item)malloc(sizeof(struct Item_));
	table_item->item_type = function_type;
	strcpy(table_item->item_name, "read");
	table_item->next_item = Symbol_Table;
	Symbol_Table = table_item;
	...
	return_type->kind = BASIC;
	return_type->u.basic = 0;
	function_type = (Type)malloc(sizeof(struct Type_));
	function_type->kind = FUNCTION;
	function_type->u.function.funcType = return_type;
	function_type->u.function.paramNum = 1;
	function_type->u.function.params = (FieldList)malloc(sizeof(struct FieldList_));

	strcpy(function_type->u.function.params->name,"param1");
	function_type->u.function.params->type = (Type)malloc(sizeof(struct Type_));
	function_type->u.function.params->type->kind = BASIC;
	function_type->u.function.params->type->u.basic = 0;
	function_type->u.function.params->next = NULL;

	table_item = (Item)malloc(sizeof(struct Item_));
	table_item->item_type = function_type;
	strcpy(table_item->item_name, "write");
	table_item->next_item = Symbol_Table;
	Symbol_Table = table_item;
}